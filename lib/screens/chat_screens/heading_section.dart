...
import 'package:cliff_pickleball/screens/common/common_selection_screen.dart';
...
import 'package:cliff_pickleball/model/chat_message_model.dart';
...
class ChatBoxHeaderSection extends StatelessWidget {
  ...
  void _copySelectedMsgData() {
    ...
    copyText(_selectedMsgData).then(
        (value) => ToastMsg.showSuccessToast('Text Copied', context: context));
  }
  ...
  void _deleteForEveryOne() async {
    ...
    ToastMsg.showErrorToast('Messages Deletion Failed', context: context);
  }
  ...
  void _deleteMyMsgOnly() async {
    ...
    if (_operationDone) {
      Provider.of<ChatBoxMessagingProvider>(context, listen: false)
          .clearSelectedMsgCollection();
      ToastMsg.showSuccessToast('Messages Deleted Successfully',
          context: context);
    } else {
      ToastMsg.showErrorToast('Failed to Delete Messages', context: context);
    }
